...
```{r}
# location of plate maps for sequenom: /dcs01/arking/arkinglab/active/projects/mito/SHAPE/SHAPE MassArray Sequenom Plate-Maps
```
...
no_alb %>% group_by(Subj.Vis) %>% summarise(Count = n()) -> hi

hi$Missing.Percent = (hi$Count/47) * 100

p = ggplot(hi, aes(Missing.Percent)) + geom_density() + geom_rug() + ggtitle('Missingness by individual sample')

labs = subset(hi, Missing.Percent < 45 | Missing.Percent > 100)
...
only_reason = subset(hi, Missing.Percent <= 100)
